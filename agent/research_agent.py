...
class ResearchAgent:
    def __init__(self, question, agent, agent_role_prompt, uploaded_files=None, websocket=None):
    ...
    async def conduct_research_from_files(self):
        file_contents = []
    ...
    async def conduct_research(self):
        if self.uploaded_files:
            await self.conduct_research_from_files()
        else:
    ...
    async def write_report(self, report_type, websocket=None):
        ...
        # if websocket is True then we are streaming GPT response, so we need to wait for the final response
